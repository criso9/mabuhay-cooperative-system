				...
				<div class="monthly-contribution">
					<table id="officer-d-cont" class="table table-striped table-bordered">
						...
						@if ($damayan->count() > 1)
						  ...
						  @endforeach
						@elseif ($damayan->count() > 0)
						  <tr>
						    <td style="text-transform: capitalize;">{{$damayan[0]->l_name}}, {{$damayan[0]->f_name}}</td>
						    <td>{{$damayan[0]->amount}}</td>
		                    <td>{{$damayan[0]->date_paid}}</td>
		                    <td>{{$damayan[0]->payment_type}}</td>
		                    <td>{{$damayan[0]->receipt_no}}</td>
		                    <td>{{$damayan[0]->updated_by}}</td>
						  </tr>
						@endif
						</tbody>
					</table>
	...
	<div id="add-contribution" class="modal fade custom-modal" tabindex="-1" role="dialog" aria-hidden="true">
		...
		<div class="modal-content custom-modal-content">
			...
			<div class="modal-body">
				{{ Form::model($damayan, array('route' => array('officer.contribution.store'), 'method' => 'post', 'class' => 'form-horizontal form-label-left')) }}
				<input type="hidden" name="_token" value="{{ csrf_token() }}">
...
$(document).ready(function() {
	...
	var fName = yearVal +  ' Damayan Contributions';
